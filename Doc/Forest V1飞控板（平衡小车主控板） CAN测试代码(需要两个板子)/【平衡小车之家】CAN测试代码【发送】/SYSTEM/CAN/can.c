...
//mode:0,普通模式;1,回环模式;
//Fpclk1的时钟在初始化的时候设置为36M,如果设置CAN_Normal_Init(1,2,3,12,1);
...
//can发送一组数据(固定格式:ID为0X601,标准帧,数据帧)	
//len:数据长度(最大为8)				     
//msg:数据指针,最大为8个字节.
//返回值:0,成功;
//		 其他,失败;
...
{	
	u8 mbox;
	u16 i=0;	  	 						       
  mbox=Can_Tx_Msg(0X601,0,0,len,msg);   
...
{	
	u8 mbox;
	u16 i=0;	  	 						       
    mbox=Can_Tx_Msg(0X701,0,0,len,msg);   
	while((Can_Tx_Staus(mbox)!=0X07)&&(i<0XFFF))i++;//等待发送结束
	if(i>=0XFFF)return 1;							//发送失败?
	return 0;										//发送成功;
}
...
/* 
*  函数名称：Can_Send_Numm(u32 id,u8* msg)
...
void CAN_SEND(int id,int a0,int a1,int a2,int a3,int a4,int a5,int a6,int a7) 
{
